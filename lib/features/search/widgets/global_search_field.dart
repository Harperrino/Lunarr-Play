...
class _GlobalSearchOverlay extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (resultState.hasError) {
      resultBody = const SizedBox(
        key: ValueKey('global-search-error-state'),
        height: 72,
        child: Center(child: Text('Suche nicht verfügbar')),
      );
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                ...
                '${indexState.readyCount}/${indexState.total}',
              ),
            ],
          ),
        ),
      );
    } else if (resultState.isLoading && !resultState.hasValue && items.isEmpty) {
      resultBody = const SizedBox(
        key: ValueKey('global-search-loading-state'),
        height: 72,
        child: Center(
          child: Row(
            mainAxisSize: MainAxisSize.min,
      ...
      resultBody = const SizedBox(
        key: ValueKey('global-search-empty-state'),
        height: 72,
...
class _SearchIndexProgressAction extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final label = 'Suchindex ${state.readyCount} von ${state.total} Playlists';
    return Semantics(
      liveRegion: true,
      label: label,
      child: Tooltip(
...
class _SearchIndexRetryAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final label =
        'Suchindex: ${state.failedCount} Playlists unvollständig. Erneut versuchen';
    return Semantics(
      button: true,
      label: label,
      child: Tooltip(
  ...
  Widget build(BuildContext context) {
    ...
    if (state.isBuilding) {
      children.addAll([
        ...
        Expanded(
          child: Text(
            'Suchindex ${state.readyCount} von ${state.total} Playlists',
            maxLines: 1,
    ...
    if (state.failedCount > 0) {
      ...
      children.add(
        Expanded(
          child: Text(
            '${state.failedCount} Playlist${state.failedCount == 1 ? '' : 's'} nicht vollständig indiziert',
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
