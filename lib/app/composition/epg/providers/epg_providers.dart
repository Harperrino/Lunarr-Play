...
/// Reactive EPG window across currently cached playlist scopes.
final epgEntriesStreamProvider = StreamProvider.autoDispose<List<EpgEntry>>((
  ...
  final playlistIds =
      ref.watch(knownEpgChannelIdsProvider).valueOrNull?.keys.toSet() ??
      const <int>{};
  return ref
      .watch(epgRepositoryProvider)
      .watchEntriesInRangeForPlaylistIds(playlistIds, start, end);
});

/// Pre-built matching index — rebuilt when EPG catalogue changes.
final epgMatchingIndexProvider = Provider.autoDispose<PlaylistEpgMatchingIndex>(
  (ref) {
    ...
    return PlaylistEpgMatchingIndex(
      knownEpgChannelIdsByPlaylist: knownIds,
      displayNamesByPlaylist: displayNames,
    );
  },
);
