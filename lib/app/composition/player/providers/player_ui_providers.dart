...
final currentProgramForEpgIdProvider = FutureProvider.autoDispose
    .family<EpgEntry?, PlaylistEpgChannelKey>((ref, key) {
      ref.watch(epgCompletionRevisionProvider);
      return ref
          .read(epgRepositoryProvider)
          .getCurrentProgram(key.playlistId, key.channelId, DateTime.now());
    });
...
final currentProgramTitleForSelectedChannelProvider =
    Provider.autoDispose<String?>((ref) {
      ...
      return ref
          .watch(
            currentProgramForEpgIdProvider(
...
final playerPanelErrorMessageProvider = Provider.autoDispose<String?>((ref) {
  return ref.watch(
    playerNotifierProvider.select(
      (playerAsync) => playerAsync.whenOrNull(
        error: (error, _) => _englishL10n.playerPanelError(error.toString()),
      ),
    ),
  );
});
...
PlayerPreparationOverlayViewModel buildPlayerPreparationOverlayViewModel({
  ...
  if (isLiveAudioAwaiting) {
    return (
      showOverlay: true,
      title: l10n.playerAudioDetectionTitle,
      subtitle: l10n.playerAudioDetectionSubtitle,
      showProgress: true,
      progressValue: null,
    );
  }
  ...
  if (isLiveAudioStabilizing) {
    return (
      showOverlay: true,
      title: l10n.playerAudioSynchronizationTitle,
      subtitle: l10n.playerAudioSynchronizationSubtitle,
      showProgress: true,
      progressValue: null,
    );
  }
  ...
  if (isLiveStartupBuffering) {
    final subtitle = bufferTargetSeconds <= 0
        ? l10n.playerInstantStartActive
        : l10n.playerStartsAtBufferTarget(
            labelForLiveStartupBufferSeconds(bufferTargetSeconds, l10n),
          );
    ...
    return (
      showOverlay: true,
      title: l10n.playerBuildingLiveBuffer,
      subtitle: subtitle,
      showProgress: true,
  ...
  return (
    showOverlay: true,
    title: l10n.playerPreparingStream,
    subtitle: hasAudioTracks
        ? l10n.playerDetectingAudio
        : l10n.playerEstablishingConnection,
    showProgress: true,
    progressValue: null,
  );
}
